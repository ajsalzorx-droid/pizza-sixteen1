...
import { ArrowUpRight, Camera, ChevronRight, MapPin, Menu, Phone, Pizza, ShoppingBag, Star, X } from "lucide-react";
...
export default function Home() {
  ...
  return <main>
    ...
    <header className={scrolled ? "scrolled" : ""}>
      ...
      <div className="nav-actions"><a className="icon-link" href="tel:+971000000000" aria-label="Call Pizza Sixteen"><Phone size={18}/></a><a className="button small" href="#order">Order now <ArrowUpRight size={17}/></a><button className="menu-button" aria-label="Open menu" onClick={() => setMenuOpen(true)}><Menu /></button></div>
    </header>
    ...
    <section className="section menu-section" id="menu">
      ...
      <div className="product-grid">{products.map((p,i)=><motion.article {...reveal} className="product" key={p[0]}>
        <span className="tag">{p[3]}</span><div className="pizza-frame"><Image src="/images/pizza-full.png" alt={p[0]} width={420} height={420}/></div><h3>{p[0]}</h3><p>{p[1]}</p><div><strong>{p[2]}</strong><button aria-label={`Add ${p[0]}`}>Add <ShoppingBag size={17}/></button></div><a href="#order">Customise →</a>
      </motion.article>)}</div>
    </section>
    ...
    <section className="order section" id="order"><motion.div {...reveal} className="section-head"><p className="eyebrow">EASY AS ONE, TWO, THREE</p><h2>FROM OUR OVEN<br/><em>TO YOUR DOOR.</em></h2></motion.div><div className="steps">{[[Pizza,"Pick Your Pizza"],[ShoppingBag,"Place Your Order"],[MapPin,"Enjoy It Hot"]].map(([Icon,label],i)=><article key={String(label)}><span>{i+1}</span><Icon size={42}/><h3>{String(label)}</h3></article>)}</div><div className="order-actions"><a className="button" href="#menu">Order online <ArrowUpRight/></a><a className="button whatsapp" href="https://wa.me/971XXXXXXXXX" target="_blank" rel="noopener noreferrer">Order on WhatsApp</a></div><p className="contact-line">Open daily 11:00 AM – 1:00 AM · +971 XX XXX XXXX</p></section>
    ...
    <section className="locations section" id="locations"><motion.div {...reveal}><p className="eyebrow">DUBAI, UAE</p><h2>FIND YOUR<br/><em>NEAREST SLICE.</em></h2><p>Sunday–Thursday: 11:00 AM–1:00 AM<br/>Friday–Saturday: 11:00 AM–2:00 AM</p><div className="hero-buttons"><a className="button" href="https://maps.google.com" target="_blank" rel="noreferrer">Get directions</a><a className="button outline" href="tel:+971000000000">Call store</a></div></motion.div><div className="map"><MapPin size={70}/><b>PIZZA<br/>SIXTEEN</b><i/><i/><i/></div></section>
    ...
    <footer><Image src="/images/pizza-sixteen-logo.png" alt="Pizza Sixteen" width={210} height={150}/><div><b>HOT • FRESH • CHEESY</b><p>Bold flavours, fresh ingredients and good times—one slice at a time.</p></div><div><a href="#menu">Menu</a><a href="#story">Our story</a><a href="#offers">Offers</a><a href="#locations">Locations</a></div><div><p>Dubai, UAE<br/>+971 XX XXX XXXX<br/>11:00 AM – 1:00 AM</p></div><small>© 2026 Pizza Sixteen. All rights reserved. · Terms · Privacy</small></footer>
    {scrolled&&<div className="floating-order"><a href="#order"><Pizza/> <span>Order now</span></a><a href="https://wa.me/971XXXXXXXXX" target="_blank" rel="noopener noreferrer" aria-label="Order on WhatsApp">W</a></div>}
  </main>;
}
